...
| **Validatables**     | let you group numerous Validatable object properties for operating on them all at once for convenience, including an extension **Populate\<T\>** to instantiate and populate a class of type T with the Validatabless Values for each property, or update an existing instance if passed in. Only the properties whose names match exactly will be updated in the class. |
...
```
:bulb: The Populate\<T\> extension will create a new instance of type T and populate its properties with the Values of the Validatable objects in our Validatables group. If you are working with an existing instance, then just pass that in for the optional model parameter to have its properties updated. If you don't like this reflection-based shortcut, then you could always set the corresponding properties manually to each Validatable object's Value property yourself.
